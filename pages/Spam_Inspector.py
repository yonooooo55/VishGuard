...
def clean_text(text):
    ...
    text = ''.join(text)  # Join the leters
    text = [word.lower() for word in text.split() if
            word.lower() not in set(stopwords.words('english'))]  # Remove common english words (I, you, we,...)
...
if st.button('Predict'):
    ...
    if input_sms == "":
        st.header(''':rainbow[Please Enter Your text] .''')
